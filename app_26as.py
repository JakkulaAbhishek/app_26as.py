...
if txt_file:
    ...
    # Improved header extraction with multiple fallback patterns
    patterns = [
        r'\d{2}-\d{2}-\d{4}\^([A-Z]{5}\d{4}[A-Z])\^[^\^]*\^(\d{4}-\d{4})\^(\d{4}-\d{4})\^?',  # standard
        r'PAN[:\s]*([A-Z]{5}\d{4}[A-Z])',                                                         # PAN: ABC...
        r'Financial\s+Year[:\s]*(\d{4}-\d{4})',                                                    # Financial Year 2024-25
        r'Assessment\s+Year[:\s]*(\d{4}-\d{4})'                                                    # Assessment Year 2025-26
    ]
    
    # Try to extract PAN, FY, AY
    for pat in patterns:
        match = re.search(pat, raw_text, re.IGNORECASE)
        if match:
            if len(match.groups()) == 1:
                # Single group - determine what it is
                grp = match.group(1)
                ...
                # Full pattern with all three
                extracted_pan = match.group(1)
                extracted_fy = match.group(2)
                extracted_ay = match.group(3)
            break
    
    # If still unknown, try scanning entire text for PAN pattern
...
def extract_26as_summary_and_section(file_bytes):
    ...
    for line in lines:
        ...
        # Look for TAN in any part
        for p in parts:
            if re.fullmatch(r"[A-Z]{4}[0-9]{5}[A-Z]", p):
                current_tan = p

        # Look for section code (e.g., 194C, 194J)
